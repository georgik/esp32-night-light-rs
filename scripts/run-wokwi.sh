...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32c3 target/${ESP_ARCH}/${BUILD_MODE}/esp32c3_night_light
else
    wokwi-server --chip esp32c3 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/esp32c3_night_light
fi
